...
const UserRegistration = () => {
  ...
  const fetchUsers = async () => {
    try {
      setLoading(true);
      const response = await axios.get('http://64.227.163.17:8000/users', {
        headers: {
      ...
      if (err.response?.status === 401) {
        try {
          ...
          const refreshResponse = await axios.post(
            'http://64.227.163.17:8000/users/refresh-token',
            {},
            {
              headers: {
          ...
          const retryResponse = await axios.get('http://64.227.163.17:8000/users');
  ...
  const handleRegister = async () => {
    ...
    try {
      const response = await axios.post(
        'http://64.227.163.17:8000/users/register',
        formData,
        {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('access_token')}`,
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        `http://64.227.163.17:8000/users/${selectedUser.id}`,
        updateData,
        {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('access_token')}`,
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(
        `http://64.227.163.17:8000/users/${id}`,
        {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('access_token')}`,
            'X-CSRF-Token': localStorage.getItem('csrf_token')
