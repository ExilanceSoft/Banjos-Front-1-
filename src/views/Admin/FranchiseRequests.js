...
const FranchiseRequests = () => {
  ...
  const fetchRequests = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await axios.get("http://64.227.163.17:8000/franchise/requests/");
  ...
  const updateRequestStatus = async (id, status) => {
    try {
      ...
      await axios.put(
        `http://64.227.163.17:8000/franchise/requests/${id}/status/${status}`
      );
  ...
  const handleDeleteRequest = async (id) => {
    try {
      setLoading(true);
      await axios.delete(`http://64.227.163.17:8000/franchise/requests/${id}`);
