...
const JobPositions = () => {
  ...
  const fetchJobPositions = async () => {
    ...
    try {
      const response = await fetch("http://64.227.163.17:8000/job-positions/");
  ...
  const fetchBranches = async () => {
    try {
      const response = await fetch("http://64.227.163.17:8000/branches/");
  ...
  const handleStatusChange = async (jobId, newStatus) => {
    try {
      ...
      const response = await fetch(`http://64.227.163.17:8000/job-positions/${jobId}`, {
        method: "PUT",
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    const url = isEditMode
      ? `http://64.227.163.17:8000/job-positions/${currentJob.id}`
      : "http://64.227.163.17:8000/job-positions/";
                  ...
                  {filteredJobs.map((job) => (
                    <CTableRow key={job.id}>
                      ...
                      <CTableDataCell>
                        {job.image_url && (
                          <CImage
                            src={`http://64.227.163.17:8000${job.image_url}`}
                            alt="Job"
                            thumbnail
                            width={50}
                            height={50}
              ...
              <CCol md={6}>
                ...
                {isEditMode && currentJob.image_url && !currentJob.image && (
                  <div className="mt-2">
                    ...
                    <CImage
                      src={`http://64.227.163.17:8000${currentJob.image_url}`}
                      alt="Current"
                      thumbnail
                      width={100}
              ...
              {viewJob.image_url && (
                <CRow className="mb-3">
                  <CCol md={12}>
                    <h6>Image</h6>
                    <CImage
                      src={`http://64.227.163.17:8000${viewJob.image_url}`}
                      alt="Job"
                      thumbnail
                      className="img-fluid"
                    />
