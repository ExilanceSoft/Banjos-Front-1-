...
const JobApplicationFollowUp = () => {
  ...
  const fetchApplications = async () => {
    ...
    try {
      ...
      const formattedData = data.map(app => ({
        ...
        resume_url: app.resume_url ? app.resume_url.replace('http://64.227.163.17:8000', '') : null
      }));
  ...
  const updateApplicationStatus = async (id, status) => {
    try {
      const response = await fetch(
        `http://64.227.163.17:8000/job-applications/${id}/status`,
        {
          method: 'PUT',
          headers: {
  ...
  const deleteApplication = async (id) => {
    if (window.confirm('Are you sure you want to delete this application? This action cannot be undone.')) {
      try {
        const response = await fetch(
          `http://64.227.163.17:8000/job-applications/${id}`, 
          { method: 'DELETE' }
        );
                                        ...
                                        <div className="col-md-6">
                                          ...
                                          {app.resume_url && (
                                            <p>
                                              ...
                                              <CButton 
                                                color="link" 
                                                href={`http://64.227.163.17:8000${app.resume_url}`} 
                                                target="_blank"
                                                className="p-0 ms-2"
                                              >
              ...
              <div className="col-md-6">
                ...
                {selectedApplication.resume_url && (
                  <div className="mb-3">
                    ...
                    <CButton 
                      color="primary" 
                      href={`http://64.227.163.17:8000${selectedApplication.resume_url}`} 
                      target="_blank"
