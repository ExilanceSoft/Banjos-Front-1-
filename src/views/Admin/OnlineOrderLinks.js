...
const OnlineOrderLinks = () => {
  ...
  const fetchLinks = async () => {
    setLoading(true);
    try {
      const response = await axios.get('http://64.227.163.17:8000/api/online-order-links/');
  ...
  const fetchBranches = async () => {
    try {
      const response = await axios.get('http://64.227.163.17:8000/branches');
  ...
  const openEditModal = (link) => {
    ...
    setLogoPreview(link.logo ? `http://64.227.163.17:8000${link.logo}` : null);
    setModalVisible(true);
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this link?')) {
      try {
        setLoading(true);
        await axios.delete(`http://64.227.163.17:8000/api/online-order-links/${id}`);
        fetchLinks();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (selectedLink) {
        await axios.put(
          `http://64.227.163.17:8000/api/online-order-links/${selectedLink.id}`,
          form,
        ...
        await axios.post(
          'http://64.227.163.17:8000/api/online-order-links/',
          form,
          { headers: { 'Content-Type': 'multipart/form-data' } }
        );
            ...
            <div className="mb-3">
              ...
              {(logoPreview || (selectedLink?.logo && !formData.logo)) && (
                <div className="mt-3">
                  ...
                  <CImage
                    thumbnail
                    src={logoPreview || `http://64.227.163.17:8000${selectedLink.logo}`}
                    width={100}
                    height={100}
